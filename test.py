...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None,
    ...
    for batch in pbar:
        ...
        # Note: gt stays as a tensor here for calculation
        gt_tensor = (batch['gt'] - gt_sub) / gt_div 
        scale = batch['scale']
        scale = scale.item()
        ...
        # Prepare GT for saving and metric calculation
        # Ensuring gt is grabbed correctly from batch for consistency
        gt = batch['gt'].cuda() # Re-fetch raw GT if needed or use gt_tensor
        # Note: The original code had a confusing np.squeeze line here. 
        # We will use the processed tensors for metrics to be safe.
        
        save_image(gt, os.path.join(output_dir, f'gt_{IDX}.png'))
        IDX = IDX+1
        
        # --- PSNR Calculation ---
        # Using the processed prediction and the normalized GT
        res = metric_fn(pred, gt_tensor)
        val_res.add(res.item())

        # --- NEW: SSIM Calculation ---
        # 1. Convert tensors to numpy (on CPU)
        # 2. Transpose from [C, H, W] to [H, W, C] for skimage
        pred_np = pred.squeeze().cpu().detach().numpy().transpose(1, 2, 0)
        gt_np = gt_tensor.squeeze().cpu().detach().numpy().transpose(1, 2, 0)
        
        # 3. Calculate SSIM
        # channel_axis=2 tells skimage that the 3rd dimension is color
        ssim = calc_ssim(pred_np, gt_np, data_range=1.0, channel_axis=2)
        val_res_ssim.add(ssim)

        if verbose:
            pbar.set_description('val PSNR: {:.4f} SSIM: {:.4f}'.format(val_res.item(), val_res_ssim.item()))
...
if __name__ == '__main__':
    ...
    # Capture both PSNR and SSIM
    res_psnr, res_ssim = eval_psnr(loader, model,
        data_norm=config.get('data_norm'),
        eval_type=config.get('eval_type'),
        eval_bsize=config.get('eval_bsize'),
        verbose=True)
    
    print('result: PSNR: {:.4f}, SSIM: {:.4f}'.format(res_psnr, res_ssim))
