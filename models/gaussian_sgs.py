...
class ContinuousGaussian(nn.Module):
    ...
    def __init__(self, encoder_spec, cnn_spec, fc_spec, **kwargs):
        ...
        # --- NEW MODULES FOR HYBRID COVARIANCE (SGS + Alpha) ---

        # 1. Alpha MLP: Learns the mixing weight (alpha) for each pixel
        # Input: 256 (features), Output: 1 (alpha value)
        alpha_mlp_spec = {'name': 'mlp', 'args': {'in_dim': 256, 'out_dim': 1, 'hidden_list': [512, 1024, 256, 128, 64]}}
        self.alpha_mlp = models.make(alpha_mlp_spec)

        # 2. SGS Bank: Learnable Gaussian Kernels (GaussianSR style)
        self.num_sgs_classes = 100
        # Learnable dictionary of covariance params (3 values per kernel)
        self.sgs_bank = nn.Parameter(torch.randn(self.num_sgs_classes, 3))

        # 3. SGS Classifier: Classifies features into the SGS bank
        self.sgs_classifier = nn.Conv2d(256, self.num_sgs_classes, kernel_size=3, padding=1)

        # -------------------------------------------------------

        # Initialize pre-defined Gaussian convariance parameter dictionaries (DDCW - Fixed Prior)
        ...
        gau_dict_cpu = torch.tensor(list(product(cho1.tolist(), cho2.tolist(), cho3.tolist())))
        gau_dict_with_zero_cpu = torch.cat((gau_dict_cpu, torch.zeros(1, 3)), dim=0)

        self.register_buffer('gau_dict', gau_dict_with_zero_cpu)
    ...
    def query_output(self, inp, scale):
        ...
        vector = self.mlp_vector(
            self.gau_dict.to(para.device))  # Transform Gaussian covariance dictionary to increase dimensions
        ...
        para = para.permute(1, 0) @ self.gau_dict.to(
            para.device)  # Compute the weighted sum of dictionary elements to get the final covariance
        para = para.reshape(bs, lr_h * lr_w * 4, -1)

        # --- BRANCH B: SGS (Learnable Bank from GaussianSR) ---
        # 1. Predict Logits using Classifier on LR Features
        sgs_logits = self.sgs_classifier(self.feat)  # [B, 100, H, W]
        sgs_logits_flat = sgs_logits.reshape(bs, self.num_sgs_classes, -1).permute(0, 2, 1).reshape(bs * lr_h * lr_w * 4,
                                                                                                    self.num_sgs_classes)

        # 2. Calculate Weights (Gumbel Softmax vs Argmax)
        if self.training:
            # Training: Use Gumbel Softmax for differentiable soft selection
            # tau=1 is standard, hard=False keeps it differentiable
            sgs_weights = F.gumbel_softmax(sgs_logits_flat, tau=1, hard=False)
        else:
            # Inference: Use Argmax for hard selection (pick the single best kernel)
            indices = torch.argmax(sgs_logits_flat, dim=-1)  # [B*num_kernels]
            sgs_weights = F.one_hot(indices, num_classes=self.num_sgs_classes).float()  # [B*num_kernels, 100]

        # 3. Weighted sum of Learnable SGS Bank
        para_sgs = sgs_weights @ self.sgs_bank  # [B*num_kernels, 3]

        # --- BRANCH C: Alpha Fusion ---
        # Predict alpha per pixel (sigmoid ensures 0 to 1 range)
        alpha = torch.sigmoid(self.alpha_mlp(para_c.reshape(-1, bs * lr_h * lr_w * 4).permute(1, 0)))  # [B*num_kernels, 1]

        # --- FUSION ---
        # Combine the two covariance predictions
        para = alpha * para_sgs + (1 - alpha) * para_c
        ...
        for i in range(bs):
            ...
            weighted_opacity = torch.ones(color_.shape[0], 1, device=self.background.device)
